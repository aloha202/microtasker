...
function system_init_content($root)
{
	...
	($root || $('body')).find('form.js-form-control').each(function(){
		$(this).find('input:text, input:password, select, textarea').addClass('form-control');
	});
...
$(function(){
    ...
    if($('.sysmesserr').size()){
        system_message_show($('.sysmesserr').text(), 'error');
		$('.sysmesserr').remove();
    }	
